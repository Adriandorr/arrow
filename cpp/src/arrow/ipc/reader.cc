...
static Status LoadArray(const Field& field, ArrayLoaderContext* context, ArrayData* out, int64_t offset = 0, int64_t length = std::numeric_limits<int64_t>::max());
...
class ArrayLoader {
 public:
  ...
  Status Visit(const DictionaryType& type) {
    RETURN_NOT_OK(
        LoadArray(*::arrow::field("indices", type.index_type()), context_, out_, _offset, _length));
...
static Status LoadArray(const Field& field, ArrayLoaderContext* context, ArrayData* out, int64_t offset, int64_t length) {
  ArrayLoader loader(field, out, context, offset, length);
  return loader.Load();
}
...
static Status LoadRecordBatchFromSource(const std::shared_ptr<Schema>& schema,
  ...
  for (int i = 0; i < schema->num_fields(); ++i) {
    ...
    RETURN_NOT_OK(LoadArray(*schema->field(i), &context, arr.get(), offset, length));
...
static inline Status ReadRecordBatch(const flatbuf::RecordBatch* metadata,
  ...
  return LoadRecordBatchFromSource(schema, metadata->length(), max_recursion_depth,
                                   &source, dictionary_memo, out, offset, length);
}
 ...
 public:
  ...
  Status ReadNext(std::shared_ptr<RecordBatch>* batch) {
    ...
    if (message->type() == Message::DICTIONARY_BATCH) {
      ...
      CHECK_HAS_BODY(*message);
    auto reader(message->body());
    return ReadRecordBatch(*message->metadata(), schema_, &dictionary_memo_, reader.get(), 
                             batch);
    }
  }
